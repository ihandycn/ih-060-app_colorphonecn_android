...
public class DisplayUtils {
    ...
    public static Bitmap drawable2BitmapWithoutLimited(Drawable drawable) {
        if (drawable == null) {
            return null;
        }
        Bitmap.Config config = drawable.getOpacity() != PixelFormat.OPAQUE ? Bitmap.Config.ARGB_8888 : Bitmap.Config.RGB_565;
        return drawable2BitmapWithoutLimited(drawable, config);
    }
    ...
    public static Bitmap drawable2Bitmap(Drawable drawable, Bitmap.Config config) {
        ...
        if (HSApplication.isDebugging && width * height > 1024 * 1024 / byteCount) {
    ...
    public static Bitmap drawable2Bitmap(Drawable drawable, int bitmapWidth, int bitmapHeight, Bitmap.Config config) {
        ...
        if (HSApplication.isDebugging && bitmapWidth * bitmapHeight > 1024 * 1024 / byteCount) {
            throw new OutOfMemoryError("创建Bitmap大小最好不要超过1M!");
        }
    ...
    public static Bitmap vectorToBitmapWithoutLimited(Context context, int vectorResId) {
        ...
        return drawable2BitmapWithoutLimited(drawable, Bitmap.Config.ARGB_8888);
    }
    ...
    public static Bitmap vectorToBitmapWithoutLimited(Context context, int vectorResId, int width, int height) {
        ...
        return drawable2BitmapWithoutLimited(drawable, width, height, Bitmap.Config.ARGB_8888);
    }

    public static Bitmap drawable2BitmapWithoutLimited(Drawable drawable, Bitmap.Config config) {
        if (drawable instanceof BitmapDrawable) {
            BitmapDrawable bitmapDrawable = (BitmapDrawable) drawable;
            if (bitmapDrawable.getBitmap() != null) {
                return bitmapDrawable.getBitmap();
        ...
        if (HSApplication.isDebugging && (width > DisplayUtils.getScreenWidth()
                || height > DisplayUtils.getScreenWithNavigationBarHeight())) {
    ...
    public static Bitmap drawable2BitmapWithoutLimited(Drawable drawable, int bitmapWidth, int bitmapHeight, Bitmap.Config config) {
        if (HSApplication.isDebugging && (bitmapWidth > DisplayUtils.getScreenWidth() || bitmapHeight > DisplayUtils.getScreenWithNavigationBarHeight())) {
            throw new OutOfMemoryError("创建Bitmap大小不能超过屏幕大小!");
        }
    ...
    public static Bitmap getBitmap(@NonNull Bitmap bitmap, int width, int height) {
    ...
    @NonNull
    public static Bitmap tintBitmap(@NonNull Bitmap bitmap, int tintColor) {
    ...
    public static int getScreenWithOutNavigationBarHeight() {
        Resources resources = HSApplication.getContext().getResources();
        return resources.getDisplayMetrics().heightPixels;
    }

    public static int getScreenWithNavigationBarHeight() {
        if (screenHeight < 0) {
            Display display = ((WindowManager) HSApplication.getContext().getSystemService(Context.WINDOW_SERVICE)).getDefaultDisplay();
    ...
    public static boolean hasPhysicalNavigationBar() {
        ...
        try {
            hasMenuKey = ViewConfiguration.get(HSApplication.getContext()).hasPermanentMenuKey();
    ...
    public static int getNavigationBarHeight() {
        if (navigationBarHeight < 0) {
            if (hasPhysicalNavigationBar()) {
                ...
                int resourceId = HSApplication.getContext().getResources()
                        .getIdentifier("navigation_bar_height", "dimen", "android");
                navigationBarHeight = HSApplication.getContext().getResources().getDimensionPixelSize(resourceId);
            }
        }
    ...
    public static float getFraction(int id) {
        return HSApplication.getContext().getResources().getFraction(id, 1, 1);
    }
    ...
    public static boolean hasDeviceNavigationBar() {
        boolean hasNavigationBar = false;
        Resources rs = HSApplication.getContext().getResources();
    ...
    public static int sp2px(float sp) {
        float density = HSApplication.getContext().getResources().getDisplayMetrics().scaledDensity;
