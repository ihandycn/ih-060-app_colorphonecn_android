        ...
        if (list != null && list.size() > 0) {
            if (isCanLoop && list.size() > 1) {
                ...
                if (position == 0) {
                    ...
                    holder.onBind(view, list.get(list.size() - 1), list.size() - 1, size);
                } else if (position == list.size() + 1) {
                    view = holder.createView(container, container.getContext(), 0);
                    holder.onBind(view, list.get(0), 0, size);
                } else {
                    view = holder.createView(container, container.getContext(), position - 1);
                    holder.onBind(view, list.get(position - 1), position - 1, size);
                }
            } else {
                view = holder.createView(container, container.getContext(), position);
                holder.onBind(view, list.get(position), position, list.size());
            }
        }
