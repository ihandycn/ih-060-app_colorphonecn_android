...
# From http://blog.brian.jp/python/png/2016/07/07/file-fun-with-pyhon.html
class Punk(object):
    ...
    def _inject_punk_chunk(self):
        ...
        if _DEBUG: print 'Hiding', (chunk_size / 1024), 'kB (', chunk_size, 'bytes)'
        ...
        if _DEBUG: print 'Injecting punk chunk'
        ...
        # Then the CRC
        self._file.write(bytearray.fromhex('AE426082'))

        if _DEBUG: print 'Punk chunk injected'

    def _read_next_chunk(self):
        chunk_size = self._read_bytes_as_int(4)
        if _DEBUG: print 'Chunk size:', chunk_size

        chunk_type = self._read_bytes_as_ascii(4)
        if _DEBUG: print 'Chunk type:', chunk_type

        if self._mode == 'encode' and chunk_type == self._END_CHUNK_TYPE:
            self._inject_punk_chunk()

            if _DEBUG: print 'Reached EOF'
            self._file.close()
        ...
        if _DEBUG: print 'CRC:', crc

        if self._mode == 'decode' and chunk_type == self._PUNK_CHUNK_TYPE:
            if _DEBUG: print "Found a punk chunk", len(content), "bytes. Writing to file"
