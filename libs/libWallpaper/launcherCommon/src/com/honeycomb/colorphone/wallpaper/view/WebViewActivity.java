...
import com.honeycomb.colorphone.wallpaper.util.LauncherActivityUtils;
...
public class WebViewActivity extends BaseAppCompatActivity implements View.OnClickListener {
    ...
    private void configAppBar() {
        toolbar = (Toolbar) findViewById(R.id.action_bar).findViewById(R.id.inner_tool_bar);
        LauncherActivityUtils.configSimpleAppBar(this, "", ContextCompat.getColor(this, R.color.material_text_black_primary), Color.WHITE, true);
    ...
    protected void onPause() {
        ...
        if (isFinishing()) {
            if (mShouldShowTitle) {
                LauncherAnalytics.logEvent("Search_WebPage_CloseIcon_Clicked");
            }
        }
    }
    ...
    private void onClickRefresh(View v) {
        if (mShouldShowTitle) {
            LauncherAnalytics.logEvent("Search_WebPage_RefreshIcon_Clicked");
        }
