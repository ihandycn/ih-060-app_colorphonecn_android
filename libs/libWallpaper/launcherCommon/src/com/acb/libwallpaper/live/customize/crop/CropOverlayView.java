...
package com.acb.libwallpaper.live.customize.crop;
...
import static com.acb.libwallpaper.live.customize.crop.CropImageOptions.CropShape.CropShape;
...
public class CropOverlayView extends View {
     ...
     */
    private CropImageOptions.Guidelines mGuidelines;

    /**
     * The shape of the cropping area - rectangle/circular.
     */
    private CropImageOptions.CropShape mCropShape;
    ...
    public void resetCropOverlayView() {
        if (initializedCropWindow) {
            setCropWindowRect(CropUtils.EMPTY_RECT_F);
            initCropWindow();
            invalidate();
        }
    }
     ...
     * The shape of the cropping area - rectangle/circular.
     */
    public CropImageOptions.CropShape getCropShape() {
        return mCropShape;
    }
     ...
     * The shape of the cropping area - rectangle/circular.
     */
    public void setCropShape(CropImageOptions.CropShape cropShape) {
        if (mCropShape != cropShape) {
            mCropShape = cropShape;
            if (Build.VERSION.SDK_INT <= Build.VERSION_CODES.JELLY_BEAN_MR1) {
                if (mCropShape == CropImageOptions.CropShape.OVAL) {
     ...
     */
    public CropImageOptions.Guidelines getGuidelines() {
        return mGuidelines;
    }
     ...
     */
    public void setGuidelines(CropImageOptions.Guidelines guidelines) {
    ...
    public void setInitialCropWindowRect(Rect rect) {
        mInitialCropWindowRect.set(rect != null ? rect : CropUtils.EMPTY_RECT);
        if (initializedCropWindow) {
            initCropWindow();
            invalidate();
    ...
    private void initCropWindow() {
        ...
        float leftLimit = Math.max(CropUtils.getRectLeft(mBoundsPoints), 0);
        float topLimit = Math.max(CropUtils.getRectTop(mBoundsPoints), 0);
        float rightLimit = Math.min(CropUtils.getRectRight(mBoundsPoints), getWidth());
        float bottomLimit = Math.min(CropUtils.getRectBottom(mBoundsPoints), getHeight());
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if (mCropWindowHandler.showGuidelines()) {
            ...
            if (mGuidelines == CropImageOptions.Guidelines.ON) {
                drawGuidelines(canvas);
            } else if (mGuidelines == CropImageOptions.Guidelines.ON_TOUCH && mMoveHandler != null) {
    ...
    private void drawBackground(Canvas canvas) {
        ...
        float left = Math.max(CropUtils.getRectLeft(mBoundsPoints), 0);
        float top = Math.max(CropUtils.getRectTop(mBoundsPoints), 0);
        float right = Math.min(CropUtils.getRectRight(mBoundsPoints), getWidth());
        float bottom = Math.min(CropUtils.getRectBottom(mBoundsPoints), getHeight());

        if (mCropShape == CropImageOptions.CropShape.RECTANGLE) {
            ...
            mPath.reset();
            if (Build.VERSION.SDK_INT <= Build.VERSION_CODES.JELLY_BEAN_MR1 && mCropShape == CropImageOptions.CropShape.OVAL) {
    ...
    public boolean onTouchEvent(MotionEvent event) {
        if (CropImageOptions.DEBUG) {
            HSLog.d(CropImageOptions.TAG, "action " + event.getAction() + " x " + event.getX() + " y " + event.getY());
        }
    ...
    private void onActionMove(float x, float y) {
        if (mMoveHandler != null) {
            ...
            mCropWindowHandler.setRect(rect);
            callOnCropWindowChanged(!rectEqual(oldRect, rect));
            invalidate();
        }
    }

    public static boolean rectEqual(RectF oldRect, RectF newRect) {
        if (Math.abs(newRect.left - newRect.left) <= 0.1
                && Math.abs(oldRect.right - newRect.right) <= 0.1
                && Math.abs(oldRect.top - newRect.top) <= 0.1
                && Math.abs(oldRect.bottom - newRect.bottom) <= 0.1) {
            return true;
        } else {
            return false;
        }
    ...
    private boolean calculateBounds(RectF rect) {
        ...
        float left = CropUtils.getRectLeft(mBoundsPoints);
        float top = CropUtils.getRectTop(mBoundsPoints);
        float right = CropUtils.getRectRight(mBoundsPoints);
        float bottom = CropUtils.getRectBottom(mBoundsPoints);
    ...
    private void callOnCropWindowChanged(boolean inProgress) {
        try {
            ...
            HSLog.e("AIC", "Exception in crop window changed: " + e);
        }
    }
    //endregion
    ...
    private class ScaleListener extends ScaleGestureDetector.SimpleOnScaleGestureListener {
        ...
        public boolean onScale(ScaleGestureDetector detector) {
            ...
            float newWidth = rect.width() * mScaleFactor;
            float newHeight = rect.height() * mScaleFactor;

            float originCenterX = rect.centerX();
            float originCenterY = rect.centerY();
            ...
            float right = CropUtils.getRectRight(mBoundsPoints);
            float bottom = CropUtils.getRectBottom(mBoundsPoints);
            if (newTop < top || newBottom > bottom || newRight > right || newLeft < left) {
                return true;
            }
            if (CropImageOptions.DEBUG) {
                HSLog.i("ScaleListener", "newTop " + newTop + " newLeft " + newLeft + " newRight " + newRight + " newBottom " + newBottom);
                HSLog.i("ScaleListener", "left " + left + " top " + top + " right " + right + " bottom " + bottom);
            }
            if (newLeft < newRight &&
                    newTop <= newBottom &&
                    newLeft >= 0 &&
