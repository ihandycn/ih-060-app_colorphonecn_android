...
public class PullableRecyclerView extends WrapRecyclerView implements Pullable {
    ...
    public boolean canPullDown() {
        ...
        mFirstVisiblePosition = getFirstCompletelyVisibleItemPosition();
        View view = lm.findViewByPosition(mFirstVisiblePosition);
        int count = getAdapter().getItemCount();
        if (0 == count) {
            // 没有item的时候也可以下拉刷新
            return true;
        } else if (null != view && mFirstVisiblePosition == 0) {
            // 滑到ListView的顶部了
            return true;
        } else
            return false;
    ...
    public int getFirstCompletelyVisibleItemPosition() {
        ...
        int firstVisibleItemPosition = 0;
        if (lm instanceof GridLayoutManager) {
            firstVisibleItemPosition = ((GridLayoutManager) lm).findFirstCompletelyVisibleItemPosition();
        } else if (lm instanceof LinearLayoutManager) {
            firstVisibleItemPosition = ((LinearLayoutManager) lm).findFirstCompletelyVisibleItemPosition();
        } else if (lm instanceof StaggeredGridLayoutManager) {
            int positions[] = new int[1];
