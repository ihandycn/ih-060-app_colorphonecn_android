...
vec4 getColorWithZoom(int index, float zoomFraction, vec2 normFragCoord)
{
    ...
    if (index == 2) return texture2D(image3, st);
    if (index == 3) return texture2D(image4, st);
    if (index == 4) return texture2D(image5, st);
    if (index == 5) return texture2D(image6, st);
    if (index == 6) return texture2D(image7, st);
