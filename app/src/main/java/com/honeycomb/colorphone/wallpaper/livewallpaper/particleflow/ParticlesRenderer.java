package com.honeycomb.colorphone.wallpaper.livewallpaper.particleflow;
...
import com.honeycomb.colorphonewallpaper.particleflow.ScriptC_particleflow;
...
public class ParticlesRenderer implements GLSurfaceView.Renderer {
    ...
    private Boolean allocationsInitialized = false;
    ...
    public void syncTouch() {
    	if(!posDirty || !allocationsInitialized) {
    		return;
    	}
    ...
    public void onSurfaceChanged(GL10 unused, int width, int height) {
        ...
        if(hasNoSizeChange() && allocationsInitialized) {
            return; // onSurfaceChanged called after resuming the activity. check before reinitialize.
        }
        initScript(false);
    }

    private boolean hasNoSizeChange() {
        return mWidth > 0 && mHeight > 0 && // Size is non-zero
                mWidth == mScript.get_width() && mHeight == mScript.get_height(); // ... and has no change
    }
    ...
    private void initAllocations(boolean forceInit) {
        if(allocationsInitialized && !forceInit) {
            return;
        }
    ...
    public void resetAttractionPoints() {
        if (allocationsInitialized && mWidth > 0 && mHeight > 0) {
    ...
    */
    public void checkGlError(String glOperation) {
        ...
        while ((error = GLES20.glGetError()) != GLES20.GL_NO_ERROR) {
            HSLog.e(TAG, glOperation + ": glError " + error);
