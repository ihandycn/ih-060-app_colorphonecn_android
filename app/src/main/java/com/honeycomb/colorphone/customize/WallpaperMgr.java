...
public class WallpaperMgr {
    ...
    private final Preferences mPrefs = Preferences.get(CustomizeConstants.CUSTOMIZE_PREFS);
    ...
    public void initLocalWallpapers(final ICustomizeService service, final WallpaperInitListener callback) {
    ...
    void saveCurrentWallpaper(final WallpaperInfo wallpaperInfo, final Runnable callback) {
        Threads.postOnSingleThreadExecutor(new Runnable() {
            @Override
            public void run() {
