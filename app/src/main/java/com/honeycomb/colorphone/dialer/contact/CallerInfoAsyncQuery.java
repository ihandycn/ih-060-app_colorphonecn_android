...
//      }
  ...
  private static class CallerInfoAsyncQueryHandler extends AsyncQueryHandler {
    ...
    protected void onQueryComplete(int token, Object cookie, Cursor cursor) {
      Log.d("CallInfoQuey", "##### onQueryComplete() #####   query complete for token: " + token);
    ...
    void updateData(int token, Object cookie, Cursor cursor) {
      try {
        Log.d("CallInfoQuey", "##### updateData() #####  for token: " + token);
        ...
        if (callerInfo == null) {
          ...
          if (cw.event == EVENT_EMERGENCY_NUMBER) {
            ...
            Log.d("CallInfoQuery", "==> Got mCallerInfo: " + callerInfo);
            ...
            if (newCallerInfo != callerInfo) {
              callerInfo = newCallerInfo;
              Log.d("CallInfoQuery", "#####async contact look up with numeric username" + callerInfo);
            }
          ...
          Log.d("CallInfoQuery", "constructing CallerInfo object for token: " + token);
