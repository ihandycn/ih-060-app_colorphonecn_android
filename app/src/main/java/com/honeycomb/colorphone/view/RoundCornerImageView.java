...
public class RoundCornerImageView extends AppCompatImageView {
    ...
    public RoundCornerImageView(Context context, AttributeSet attrs, int defStyle) {
        ...
        TypedArray a = context.obtainStyledAttributes(attrs,
                R.styleable.RoundCornerImageView, defStyle, 0);

        final int index = a.getInt(R.styleable.RoundCornerImageView_android_scaleType, -1);
        if (index >= 0) {
            setScaleType(sScaleTypeArray[index]);
        }

        mLeftTopCornerRadius = a.getDimensionPixelSize(
                R.styleable.RoundCornerImageView_round_left_top_corner_radius, 0);
        mRightTopCornerRadius = a.getDimensionPixelSize(
                R.styleable.RoundCornerImageView_round_right_top_corner_radius, 0);
        mLeftBottomCornerRadius = a.getDimensionPixelSize(
                R.styleable.RoundCornerImageView_round_left_bottom_corner_radius, 0);
        mRightBottomCornerRadius = a.getDimensionPixelSize(
                R.styleable.RoundCornerImageView_round_right_bottom_corner_radius, 0);
        ...
        mBorderWidth = a.getDimensionPixelSize(
                R.styleable.RoundCornerImageView_round_border_width, 0);
        if (mBorderWidth < 0) {
            throw new IllegalArgumentException("border width cannot be negative.");
        }

        mBorderColor = a
                .getColorStateList(R.styleable.RoundCornerImageView_round_border_color);
        if (mBorderColor == null) {
            mBorderColor = ColorStateList.valueOf(DEFAULT_BORDER_COLOR);
        }

        isOval = a.getBoolean(R.styleable.RoundCornerImageView_round_oval, false);
        a.recycle();
    ...
    private Drawable resolveResource() {
        ...
        if (mResource != 0) {
            try {
                ...
                HSLog.i(TAG, "Unable to find resource: " + mResource);
                // Don't try again.
                mResource = 0;
            }
        }
        ...
        public static Drawable fromDrawable(Drawable drawable, Resources r) {
            if (drawable != null) {
                ...
                if (bm != null) {
                    ...
                    HSLog.i(TAG, "Failed to create bitmap from drawable!");
                }
            }
            return drawable;
        }
