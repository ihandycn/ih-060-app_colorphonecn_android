...
public class ProgressWheel extends View {
    ...
    private final int barMaxLength = 360;
    ...
    private double barSpinCycleTime = 700;
    ...
    private float finishSpeed = 230.0f;
    ...
    private void parseAttributes(TypedArray a) {
        ...
        circleRadius =
                (int) a.getDimension(R.styleable.ProgressWheel_circleRadius, circleRadius);
        ...
        float baseSpinSpeed =
                a.getFloat(R.styleable.ProgressWheel_spinSpeed, spinSpeed / 360.0f);
        spinSpeed = baseSpinSpeed * 360;

        barSpinCycleTime =
                a.getInt(R.styleable.ProgressWheel_barSpinCycleTime, (int) barSpinCycleTime);
    ...
    public double getBarSpinCycleTime() {
        return barSpinCycleTime;
    }

    public void setBarSpinCycleTime(double barSpinCycleTime) {
        this.barSpinCycleTime = barSpinCycleTime;
    }
    ...
    public void setProgress(float progress) {
        ...
        if (isSpinning) {
            ...
            if (mIsDebug) {
                HSLog.i(TAG, barGrowingFromFront + "  progress  " + mProgress + "  length  " + length);
            }
            runCallback();
        }
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if (isSpinning) {
            ...
            float length = barLength + barExtraLength;
            mStartAngle = length;
            if (isPost && !isConsumed) {
                float sweepAngle = (float) ((1.0f - Math.pow(1.0f - (mProgress) / 360.0f, 2.0f)) * 360.0f);
                if (Math.abs(sweepAngle - mStartAngle) > 20) {
                    postProgress(100.0f);
                } else {
                    setProgress(100.0f);
                }
            ...
            //Log.i(TAG + 0, from + "  " + length + "  barLength " + barLength + "  barExtraLength" + barExtraLength);
            if (isInEditMode()) {
                from = 0;
                length = 135;
            }

            canvas.drawArc(circleBounds, from - 155, length, false, barPaint);
        } else {
            ...
            if (mProgress != mTargetProgress) {
                ...
                mProgress = Math.min(mProgress + deltaNormalized + 2.0f, mTargetProgress);
                lastTimeAnimated = SystemClock.uptimeMillis();
            }
            ...
            float sweepAngle = mStartAngle;
            if (!linearProgress) {
                float factor = 2.0f;
                //startAngle = (float) (((1.0f - Math.pow(1.0f - (mProgress) / 360.0f, 2.0f * factor)) * 360.0f) + 5.0f);
                // = (float) (1.0f - Math.pow(1.0f - mProgress / 360.0f, 2.0f * factor)) * 360.0f;
                sweepAngle = (float) ((1.0f - Math.pow(1.0f - (mProgress) / 360.0f, factor)) * 360.0f);
                if (sweepAngle < mStartAngle) {
                    sweepAngle = mStartAngle;
                }
                // Log.i(TAG + 0, mProgress + "  " + startAngle + "  " + sweepAngle);
            ...
            if (mIsDebug) {
                HSLog.i(TAG + 0, "from  " + mProgress + "  " + "     sweepAngle  " + sweepAngle + "  mStartAngle  " + mStartAngle);
            }

            //Log.i(TAG + 1, mProgress + "  " + startAngle + "  " + sweepAngle);
            canvas.drawArc(circleBounds, startAngle - 155, sweepAngle, false, barPaint);
        }
    ...
    private void updateBarLength(long deltaTimeInMilliSeconds) {
        if (pausedTimeWithoutGrowing >= pauseGrowingTime) {
            ...
            //Log.w(TAG, "   zhouqi " + (timeStartGrowing / barSpinCycleTime + 1) + "  timeStartGrowing  " + timeStartGrowing + " barSpinCycleTime  " + barSpinCycleTime);
            ...
            //Log.w(TAG, " distance " + distance + " barExtraLength " + barExtraLength);
        } else {
    ...
    public void setFinishSpeed(float finishSpeed) {
    ...
    public Parcelable onSaveInstanceState() {
        ...
        ss.startAngle = this.mStartAngle;
        ss.isPost = this.isPost;
        ss.isConsumed = this.isConsumed;
    ...
    public void onRestoreInstanceState(Parcelable state) {
        ...
        this.isPost = ss.isPost;
        this.isConsumed = ss.isConsumed;
        this.mStartAngle = ss.startAngle;
    ...
    static class WheelSavedState extends BaseSavedState {
        ...
        public void writeToParcel(Parcel out, int flags) {
            ...
            out.writeByte((byte) (isConsumed ? 1 : 0));
            out.writeByte((byte) (isPost ? 1 : 0));
        }
    }
}
